...
#include "ShaderInPluginsBlueprintLibrary.generated.h"
...
class UShaderInPluginsBlueprintLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_UCLASS_BODY()


	/** Returns the overscan factor required for the undistort rendering to avoid unrendered distorted pixels. */
	UFUNCTION(BlueprintPure, Category = "ShaderInPlugins | Lens Distortion")
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "ShaderInPlugins | Lens Distortion")
	...
	static bool EqualEqual_CompareLensDistortionModels(
		const FShaderInPluginsCameraModel& A,
		const FShaderInPluginsCameraModel& B)
	{
		return A == B;
	}
