...
/** Mathematic camera model for ShaderInPlugins | Lens Distortion/undistortion.
 *
 * Camera matrix =
 *  | F.X  0  C.x |
 *  |  0  F.Y C.Y |
...
struct SHADERSINPLUGINS_API FShaderInPluginsCameraModel
{
	GENERATED_USTRUCT_BODY()
	FShaderInPluginsCameraModel()
	{
		K1 = K2 = K3 = P1 = P2 = 0.f;
		F = FVector2D(1.f, 1.f);
	...
	/** Radial parameter #1. */
	UPROPERTY(Interp, EditAnywhere, BlueprintReadWrite, Category = "ShaderInPlugins | Lens Distortion|Camera Model")
	float K1;
	...
	/** Compare two ShaderInPlugins | Lens Distortion models and return whether they are equal. */
	bool operator == (const FShaderInPluginsCameraModel& Other) const
	{
		return (
			K1 == Other.K1 &&
			K2 == Other.K2 &&
	...
	/** Compare two ShaderInPlugins | Lens Distortion models and return whether they are different. */
	bool operator != (const FShaderInPluginsCameraModel& Other) const
	{
		return !(*this == Other);
	}
