...
/**
 * Internal intermediary structure derived from FShaderInPluginsCameraModel by the game thread
...
struct FCompiledCameraModel
{
	...
	FShaderInPluginsCameraModel OriginalCameraModel;
...
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationVS, TEXT("/Plugin/ShadersInPlugins/Private/MyShader.usf"), TEXT("MainVS"), SF_Vertex)
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationPS, TEXT("/Plugin/ShadersInPlugins/Private/MyShader.usf"), TEXT("MainPS"), SF_Pixel)
...
FVector2D FShaderInPluginsCameraModel::UndistortNormalizedViewPosition(FVector2D EngineV) const
{
...
float FShaderInPluginsCameraModel::GetUndistortOverscanFactor(
	float DistortedHorizontalFOV, float DistortedAspectRatio) const
{
	...
	if (*this == FShaderInPluginsCameraModel())
	{
		return 1.0f;
	}
...
void FShaderInPluginsCameraModel::DrawUVDisplacementToRenderTarget(
	UWorld* World,
